...
public class WordDictionary {
    private HashMap<Character, HashMap<Integer, ArrayList<String>>> _set;
    ...
    private Set<String> intersect(Set<String> set1, Set<String> set2) {
        var intersectedSet = new HashSet<String>();
        Set<String> minSet;
        Set<String> bigSet;
    ...
    public Set<String> getWordsByLetter(Character letter, Integer position) {
        var pos = _set.get(letter).get(position);
    ...
    public Set<String> getWordsByLetter(Character letter, List<Integer> positions) {
        var set = getWordsByLetter(letter, positions.get(0));
        for (int i = 1; i < positions.size(); ++i) {
            var newSet = getWordsByLetter(letter, positions.get(i));
            if (newSet.size() == 0) {
                return newSet;
            }
